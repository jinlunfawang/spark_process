...
public class SyncToRedisUDF {
    private static final Logger logger = LoggerFactory.getLogger(NER2Redis.class);
    public static final String LOG_DATE_HOUR = "log_date_hour";
    ...
    private static void setupOptions() {
        ...
        options.addOption(OptionBuilder.withLongOpt(LOG_DATE_HOUR).withDescription("log_date")
    ...
    public static void main(String[] args) throws IOException {
        ...
        String log_date_hour = null;

        try {
            CommandLine line = cmdParser.parse(options, otherArgs);
            if (line.hasOption(LOG_DATE_HOUR)) {
                log_date_hour = line.getOptionValue(LOG_DATE_HOUR);
            } else {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("SyncToRedisUDF")
                .config(new SparkConf())
                .enableHiveSupport()
                .getOrCreate();
        Dataset<Row> redisDF = spark.sql("select concat('account_id_labels_', account_id) as redis_key, concat_ws(',', collect_set(cast(label_id as string))) as redis_value from bili_sycpb.dwd_prty_acc_acc_account_label_mapping_a_hr where concat(log_date, log_hour) = " + log_date_hour +  " group by account_id" );
        redisDF.show(10);

...
public class CleanPidEvenRedisNew {
    private static final Logger logger = LoggerFactory.getLogger(CleanPidEvenRedisNew.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("CleanPidRedis")
                .config(new SparkConf())
                .enableHiveSupport()
                .getOrCreate();
        // 清理偶数秒钟的时间
        String SQL = "select rediskey from " + inputTable + " where log_date=" + log_date + " and cast(substring(rediskey,14,1) as int)%2=0";
    ...
    private static void parseIterator(Iterator<Row> iterator) {
        JedisCluster jc = new JedisCluster(parseHosts(Constants.PID_REDIS_CLEAN_EVEN_ADDRESS));
